...
export async function POST(request: Request) {
  ...
  if (!accountId || !privateKey || !topicId)
    return NextResponse.json(
      { error: "Configure HEDERA_ACCOUNT_ID, HEDERA_PRIVATE_KEY, and HEDERA_RECEIPT_TOPIC_ID" },
      { status: 503 },
    );
  const body = await request.json();
  if (!body.invoiceId || !body.txId)
    return NextResponse.json({ error: "invoiceId and txId are required" }, { status: 400 });
